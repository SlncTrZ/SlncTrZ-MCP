...
describe("stdio adapter (integration: real spawn, no fake)", () => {
  ...
  it("injects resolved env credentials only through the manifest env allowlist", async () => {
    const dir = await makeTempDir();
    const provider = await writeMockProvider(dir, "echo_env");
    const manifest = await compileExtensionManifest({
      id: "mock",
      ...
      tools: [{ canonicalId: "mock.echo", riskClass: "read" }],
      envAllowlist: ["MOCK_TOKEN"],
      credentialRefs: ["mock-token"],
      startupTimeoutMs: 5000,
      requestTimeoutMs: 5000
    });
    const adapter = createStdioAdapter(manifest, [
      { kind: "env", name: "MOCK_TOKEN", value: "FROM_CREDENTIAL_STORE" }
    ]);
    await adapter.start();
    const result = await adapter.callTool("echo", {}, {});
    expect(result.text).toBe("env=FROM_CREDENTIAL_STORE;other=none");
    await adapter.stop();
  });
  ...
  it("drains stderr but tears down a provider that exceeds its output cap", async () => {
    ...
    const adapter = createStdioAdapter(manifest);
    await adapter.start();
    await expect(adapter.callTool("echo", {}, {})).rejects.toMatchObject({
  ...
  it("keeps an administratively disabled provider declared but unavailable", async () => {
    const dir = await makeTempDir();
    const provider = await writeMockProvider(dir, "ok");
    const runtime = await createExtensionRuntimeCatalog(
      await compileExtensionRegistry([
    ...
    expect(runtime.registry.lookupProvider("mock")).toHaveLength(1);
