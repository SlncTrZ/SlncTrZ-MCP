...
describe("MCP credential activation lifecycle", () => {
    ...
    try {
      ...
      const providerService = createMcpProviderService({
        ...
        isActiveProviderReady: (providerId, activeVersion) =>
          activeVersion === String(generation) && active?.isReady(providerId) === true,
        policyStore: {
          async reload() {
  ...
  it("restores OLD when NEW probes successfully but the activated provider is unavailable", async () => {
    const root = await mkdtemp(join(tmpdir(), "slnctrz-credential-recovery-"));
    let active: ExtensionRuntimeCatalog | undefined;
    try {
      const marker = join(root, "fail-new.marker");
      const script = join(root, "provider.cjs");
      await writeFile(
        script,
        [
          'const { existsSync } = require("node:fs");',
          'if (process.env.MOCK_TOKEN === "NEW" && existsSync(process.argv[2])) process.exit(23);',
          'process.stdin.setEncoding("utf8");',
          'let carry = "";',
          'process.stdin.on("data", (chunk) => {',
          "  carry += chunk;",
          ...
          "    const request = JSON.parse(line);",
          "    let result;",
          '    if (request.method === "server/discover") result = { supportedVersions: ["2026-07-28"] };',
          '    else if (request.method === "initialize") result = { protocolVersion: "2025-11-25" };',
          '    else if (request.method === "tools/list") result = { tools: [{ name: "echo" }] };',
          '    else result = { content: [{ type: "text", text: process.env.MOCK_TOKEN ?? "missing" }] };',
          '    process.stdout.write(JSON.stringify({ jsonrpc: "2.0", id: request.id, result }) + "\\n");',
          "  }",
      ...
      active = await buildRuntime();
      expect((await active.provider("demo")?.invoke("echo", {}))?.text).toBe("OLD");
      ...
      const wrapped = {
        ...providerService,
        async discoverCandidate(candidate: ExtensionManifestV1) {
          const discovery = await providerService.discoverCandidate(candidate);
          await writeFile(marker, "fail NEW activation", "utf8");
          return discovery;
        }
      };
      const orchestrator = createMcpOwnerOrchestrator({
      ...
      expect(result).toMatchObject({
        status: "rolled_back",
        failedStep: "mcp_provider_activation_unavailable",
        recovery: { safeToRetry: true }
      });
      expect(active.isReady("demo")).toBe(true);
      expect((await active.provider("demo")?.invoke("echo", {}))?.text).toBe("OLD");
      const restored = (await providerStore.list())[0];
      expect(restored?.manifest.credentialRefs).toEqual([oldRef]);
      expect(await credentialStore.resolve([oldRef])).toEqual([
        { kind: "env", name: "MOCK_TOKEN", value: "OLD" }
      ]);
    } finally {
      await active?.stop();
      await rm(root, { recursive: true, force: true });
