...
describe("core.read (contained filesystem read)", () => {
  ...
  it("rejects default-deny, absolute, traversal, and missing paths", async () => {
    ...
    await writeFile(join(root, "x.txt"), "ok", "utf8");
    // Absolute path inside the root is now allowed (containment-checked).
    const absInside = await readContainedFile(root, join(root, "x.txt"));
    expect(absInside.content).toBe("ok");
    // Absolute path OUTSIDE the root is denied (escapes the root).
