...
describe("core.write (atomic contained filesystem write)", () => {
  ...
  it("previews only when dryRun is true", async () => {
    const root = await makeTempDir();

    const result = await writeContainedFile(root, "preview.txt", "hello", { dryRun: true });
    expect(result.applied).toBe(false);
    expect(result.created).toBe(true);
    await expect(lstat(join(root, "preview.txt"))).rejects.toMatchObject({ code: "ENOENT" });
  });
  ...
  it("requires the expected hash to overwrite and preserves file mode", async () => {
    ...
    const preview = await writeContainedFile(root, "existing.txt", "new", { dryRun: true });
  ...
  it("serializes concurrent replacements against one expected hash", async () => {
    ...
    const preview = await writeContainedFile(root, "existing.txt", "candidate", { dryRun: true });
    const expectedSha256 = preview.previousSha256;
    if (expectedSha256 === undefined) throw new Error("preview omitted previousSha256");
