...
describe("core.search (contained filesystem search)", () => {
  it("returns deterministic recursive filename matches", async () => {
    ...
    expect(result.matches).toEqual(["a.txt", "z.txt", "sub/b.txt"]);
    expect(result.truncated).toBe(false);
    expect(result.scannedEntries).toBeGreaterThan(0);
  });

  it("treats `*` as match-all and `*.ext` as a glob, plain patterns as substring", async () => {
    const root = await makeTempDir();
    await writeFile(join(root, "a.py"), "x", "utf8");
    await writeFile(join(root, "b.js"), "x", "utf8");
    await writeFile(join(root, "c.py"), "x", "utf8");
    await mkdir(join(root, "sub"));
    await writeFile(join(root, "sub", "d.py"), "x", "utf8");

    const all = await searchContainedFiles(root, "*");
    expect(all.matches).toEqual(["a.py", "b.js", "c.py", "sub", "sub/d.py"]);

    const py = await searchContainedFiles(root, "*.py");
    expect(py.matches).toEqual(["a.py", "c.py", "sub/d.py"]);

    const plain = await searchContainedFiles(root, "a");
    expect(plain.matches).toEqual(["a.py"]);
  });
