...
describe("extension manifest (typed strict JSON schema)", () => {
  ...
  it("accepts HTTP endpoints on loopback hosts (controlled exception)", async () => {
    for (const endpoint of [
      ...
      await expect(
        compileExtensionManifest({
          ...
          version: "1.0.0",
          endpoint,
          tools: [{ canonicalId: "x.y", riskClass: "read" }]
        } as ExtensionManifestV1)
      ).resolves.toBeDefined();
    }
  });

  it("still rejects HTTP endpoints on non-loopback hosts", async () => {
    for (const endpoint of ["http://x.example.com/mcp", "http://192.168.1.10/mcp"]) {
      await expect(
        compileExtensionManifest({
          id: "pi-core",
          transport: "streamable-http",
  ...
  it("exposes the bare tool name separately from the namespaced canonical id", async () => {
    const compiled = await compileExtensionManifest({
      id: "github",
      transport: "stdio",
      version: "1.0.0",
      command: "/usr/bin/github-mcp",
      tools: [{ canonicalId: "github.search_repos", riskClass: "read" }]
    } as ExtensionManifestV1);
    expect(compiled.tools[0]?.canonicalId).toBe("github.search_repos");
    expect(compiled.tools[0]?.exposedName).toBe("search_repos");
  });

  it("preserves tool and provider descriptions for model context", async () => {
    const compiled = await compileExtensionManifest({
      id: "github",
      transport: "stdio",
      version: "1.0.0",
    ...
    expect(compiled.description).toBe("GitHub MCP server for repository access");
    expect(compiled.tools[0]?.description).toBe("Search repositories");
  });
  ...
  it("uses large-workload provider payload defaults", async () => {
    const compiled = await compileExtensionManifest({
      id: "large",
      transport: "stdio",
      version: "1.0.0",
      command: "/usr/local/bin/large-mcp",
      tools: [{ canonicalId: "large.run", riskClass: "read" }]
    });
    expect(compiled.maxOutputBytes).toBe(8 * 1_048_576);
    expect(compiled.maxMessageBytes).toBe(8 * 1_048_576);
  });

  it("rejects limits above the hard ceiling", async () => {
    const cases = [
      { maxOutputBytes: 17 * 1_048_576 },
      { maxMessageBytes: 17 * 1_048_576 },
      { maxQueue: 513 },
      { maxRestarts: 17 },
