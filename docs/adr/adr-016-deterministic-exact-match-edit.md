...
> Current-contract note (2026-09-04): exact-match/hash/ambiguity semantics remain active, but the historical dry-run default is superseded. `core.edit` now applies when `dryRun` is omitted or false; pass `dryRun:true` explicitly for preview. Living docs/code take precedence for invocation defaults.
...
- Historical decision at the time: dry-run was the default. This invocation default is superseded as noted above; current calls apply unless `dryRun:true` is passed explicitly.
