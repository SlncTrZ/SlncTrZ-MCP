...
> Current-contract note (2026-09-04): the atomic/hash/boundary decisions remain active, but the historical "writes default to dry-run" behavior is superseded. `core.write` now applies when `dryRun` is omitted or false; pass `dryRun:true` explicitly for preview. Living docs/code take precedence for invocation defaults.
...
- Historical decision at the time: writes defaulted to dry-run. This invocation default is superseded as noted above.
