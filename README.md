...
- **Gemini custom MCP / Gemini Spark** — use the configured Client ID/Secret (the Client ID does not have to remain `slnctrz-mcp`). Gemini supplies a user-bound callback when it opens `/authorize`. If that exact callback has not been seen before, the consent page shows the full Client ID and callback; entering the Owner Passphrase and approving registers that exact callback and completes authorization in one step. No callback copy/paste, `client.env` edit, wildcard, or gateway restart is required. Only strict `https://oauth-redirect.googleusercontent.com/r/user_bound_custom-mcp-...` candidates are eligible; every approved URI is persisted and matched exactly.
