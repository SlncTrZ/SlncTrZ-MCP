...
.conn-link{border:0;background:transparent;color:#697586;font-size:.76rem;font-weight:650;cursor:pointer;padding:.15rem 0}
.conn-link:hover{color:#2f5a9e;text-decoration:underline}
.conn-controls{display:flex;gap:.4rem 1rem;flex-wrap:wrap;align-items:flex-end}
.conn-group{display:flex;gap:.4rem;align-items:flex-end}
.conn-group-label{font-size:.62rem;font-weight:700;letter-spacing:.09em;text-transform:uppercase;color:#697586;padding-bottom:.6rem}
.conn-group select{width:auto}
...
.empty{color:#6b7480}
.note{color:#9aa4b2}
.conn-group-label{color:#8f99a8}.conn-badge{color:#8f99a8;border-color:#333a44}.conn-badge.full{color:#5b8def;border-color:#5b8def}.conn-link{color:#9aa4b2}.conn-link:hover{color:#e6e8eb}
}
...
<section class="card"><div class="toolbar"><div class="grow"><h1>Connections</h1><span class="muted">Tool surface per OAuth grant</span></div></div><div id="connections"></div><p class="note">Gateway-only hides coding tools. Rename changes display names only.</p></section>
...
function renderConnections(list){const el=q('connections');el.replaceChildren();for(const c of list||[]){const grantId=String(c.grantId||c.connectionId||'');const label=String(c.label||c.clientId||'OAuth client');const profile=c.surfaceProfile||'full';const row=document.createElement('div');row.className='item conn-item';const main=document.createElement('div');main.className='grow';const top=document.createElement('div');top.className='conn-top';const title=document.createElement('div');title.className='conn-title';title.textContent=label;title.title=label;const badge=document.createElement('span');badge.className='conn-badge'+(profile==='full'?' full':'');badge.textContent=profile==='full'?'Full':'Gateway-only';top.append(title,badge);const meta=document.createElement('div');meta.className='muted mono conn-meta';meta.title='grant '+grantId;meta.textContent='grant '+(grantId.length>14?grantId.slice(0,8)+'…'+grantId.slice(-4):grantId)+' · '+String(c.clientId||'')+' · '+String((c.scopes||[]).join(' '));const copy=document.createElement('button');copy.type='button';copy.className='conn-link';copy.textContent='Copy grant ID';copy.setAttribute('aria-label','Copy grant ID for '+label);copy.onclick=async()=>{try{await navigator.clipboard.writeText(grantId);copy.textContent='Copied';setTimeout(()=>{copy.textContent='Copy grant ID'},1200)}catch{}};main.append(top,meta,copy);const controls=document.createElement('div');controls.className='conn-controls';const surface=document.createElement('div');surface.className='conn-group';const surfaceLabel=document.createElement('span');surfaceLabel.className='conn-group-label';surfaceLabel.textContent='Surface';const select=document.createElement('select');select.setAttribute('aria-label','Tool surface for '+label);for(const value of ['full','gateway-only']){const option=document.createElement('option');option.value=value;option.textContent=value==='full'?'Full':'Gateway-only';select.appendChild(option)}select.value=profile;const apply=btn('Apply','btn-approve',async()=>{await api('/owner/api/connections/profile',{method:'PUT',body:JSON.stringify({grantId,surfaceProfile:select.value})});await refresh()});const makeDefault=btn('Default','btn-deny',async()=>{await api('/owner/api/connections/default',{method:'PUT',body:JSON.stringify({clientId:c.clientId,surfaceProfile:select.value})});await refresh()});makeDefault.title='Save as default for future grants of this client';surface.append(surfaceLabel,select,apply,makeDefault);const naming=document.createElement('div');naming.className='conn-group';const namingLabel=document.createElement('span');namingLabel.className='conn-group-label';namingLabel.textContent='Name';const rename=document.createElement('input');rename.className='conn-rename';rename.value=String(c.label||'');rename.maxLength=64;rename.placeholder='Agent name';rename.setAttribute('aria-label','Display name for '+label);const renameBtn=btn('Rename','btn-deny',async()=>{await api('/owner/api/connections/label',{method:'PUT',body:JSON.stringify({grantId,label:rename.value})});await refresh()});naming.append(namingLabel,rename,renameBtn);controls.append(surface,naming);row.append(main,controls);el.appendChild(row)}if(!(list||[]).length){const empty=document.createElement('div');empty.className='empty';empty.textContent='No active OAuth connections.';el.appendChild(empty)}}
