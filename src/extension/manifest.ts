...
import { type RiskClass, isValidCanonicalId, toolNameOf } from "../kernel/tool-identity.js";
...
export const HARD_MAX_OUTPUT_BYTES = 16 * 1_048_576;
export const HARD_MAX_MESSAGE_BYTES = 16 * 1_048_576;
...
const manifestSchema = z
  .object({
    ...
    // Empty tool sets are allowed only for owner-side discovery probes. The compiled
    // registry rejects persisted/runtime providers with zero declared tools.
    tools: z.array(toolRecordSchema).max(MAX_TOOLS_PER_EXTENSION),
...
export async function compileExtensionManifest(
  ...
  if (parsed.data.transport === "streamable-http") {
    ...
    // is a loopback address (see adr-025). Fail-closed for every non-loopback host.
  ...
  const tools: CompiledExtensionTool[] = parsed.data.tools.map((tool) => {
    ...
    const exposedName = toolNameOf(tool.canonicalId);
  ...
  return Object.freeze({
    ...
    maxOutputBytes: parsed.data.maxOutputBytes ?? 8 * 1_048_576,
    maxMessageBytes: parsed.data.maxMessageBytes ?? 8 * 1_048_576,
