...
export const DEFAULT_MAX_EDIT_OPERATIONS = 256;
export const DEFAULT_MAX_EDIT_BYTES = 8 * 1_048_576;
export const DEFAULT_MAX_EDIT_INPUT_BYTES = 8 * 1_048_576;
export const DEFAULT_MAX_DIFF_BYTES = 1_048_576;
...
function buildDiff(
  ...
  for (const edit of resolved) {
    while (lineCursor < edit.start) {
      const newline = content.indexOf("\n", lineCursor);
      if (newline === -1 || newline >= edit.start) break;
      line += 1;
 ...
 * Mutates by default; `options.dryRun: true` returns a preview.
 */
